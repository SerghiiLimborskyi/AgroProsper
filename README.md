...
Це колекція Soulbound Token бейджів для AgroProsper:

- Створено у SVG — легко вбудовуються у NFT-платформи
- `tokenURI.json` — опис + атрибути + IPFS-шлях до зображення
- Можна хостити на IPFS або GitHub Pages

👉 Використання:
- Деплой смартконтракт `AgroBadge.sol`
- Передай `mint(user, uri)` для кожного токену
- Усі токени Soulbound (непереносимі)
