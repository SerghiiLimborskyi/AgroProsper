...
# 🌾 AgroProsper — DAO WebApp (v2.0, Reboot July 2025)

AgroProsper — це децентралізована платформа для голосувань, фермерських ідентифікацій, PDF-гайдів, Telegram-інтеграцій, NFT-доступу і DAO-карток.

🌐 Сайт працює через GitHub Pages з папки `/docs/` →  
🔗 https://serghiilimborskyi.github.io/AgroProsper/

---
# 🌱 AgroProsper — DAO WebApp v6

AgroProsper — це AI-платформа, що дозволяє фермерам:
- аналізувати ринок,
- контролювати фінанси,
- брати участь у DAO-голосуваннях,
- взаємодіяти з Web3 через SBT бейджі та NFT-картки.

👉 [Live Demo: start.html](https://serghiilimborskyi.github.io/AgroProsper/start.html)

---

## 🧠 DAO Модель мислення

Схема: **Мислення → Документація → Верифікація → Голосування → Публікація → Дія**

Кожна роль у DAO діє через `start.html` — інтерактивну карту пам’яті з вузлами A, B, C, DAO-card, зовнішніми B1/C1, та логічними переходами.

---
         README.md v6
## 🗺️ Карта пам’яті (v6)

- 🔵 Вузли A, B, C — імпульси мислення
- 🟣 DAO-Card — ідентифікатор дії
- 🧩 Зовнішні блоки — B1, C1 (інтеграції)
- 🔗 Стрілки дій — логіка переходу між станами
- 🖼️ SVG-візуалізація інтерактивна

> Усі вузли інтерактивні і зв’язані з DAO-логікою.
...
```bash
AgroProsper/
├── docs/                 # 🌐 WebApp DAO — головна директорія
│   ├── index.html        # 🎯 Головна стартова сторінка WebApp
│   ├── start.html        # 🎬 Старт панелі DAO
│   ├── vote.html         # 🗳️ Голосування з OTP
│   ├── card.html         # 📘 DAO-картка учасника
│   ├── gov-access.html   # 🔐 Панель адміністрування
│   ├── qr.html           # 📲 Telegram / QR команди
│   ├── media-center.html # 🎞️ Трейлери, PDF, ZIP
│   ├── assets/           # 🎨 Стилі, логотипи, фон
│   ├── scripts/          # ⚙️ JavaScript логіка WebApp
│   ├── exports/          # 📦 Архіви WebApp, трейлери, гіди
│   └── docs/             # 📄 Ліцензії, whitepaper, токеноміка
├── .github/              # 🤖 GitHub Actions / автодеплой
│   └── workflows/
│       └── deploy.yml    # 🔁 Автозбірка і деплой у `gh-pages`
├── README.md             # 📘 Опис проєкту AgroProsper DAO
└── LICENSE               # ⚖️ Ліцензія DAO (якщо буде додана)
...
# 🛡️ AgroProsper DAO – Модуль безпеки та політики

Цей репозиторій є частиною екосистеми AgroProsper — AI-платформи для фермерських DAO. Він містить шаблони безпеки, адміністративні політики, код смарт-контрактів для управління доступом та інтеграції з Telegram/Microsoft.
...
| Каталог                     | Вміст                                  |
|-----------------------------|----------------------------------------|
| `.github/workflows/`        | CI/CD деплой (`deploy.yml`)            |
| `Контракти/`                | `AgroChamp.sol` — DAO SBT-контракт     |
| `Астро/макети/`             | Структура WebApp через Astro           |
| `Активи/CSS/`               | Стиль `custom.css`                     |
| `Інтерфейс користувача/`    | MS інтеграція та безпечний UI          |
| `Комплект адміністратора/`  | Telegram-файли, командний центр        |
| `Документи/`                | `SECURITY.md`, `governance.md`, ліцензія|

---

## 🔐 Мета репозиторію

✅ Забезпечити основу для аудиту безпеки DAO  
✅ Задати правила управління через `governance.md`  
✅ Імплементувати SBT-ідентифікацію для адмінів (`AgroChamp.sol`)  
✅ Створити `SECURITY.md` з протоколами реагування  
✅ Надати інтерфейс для звʼязку через Telegram/MS Teams  
✅ Формувати white-label політики для інших фермерських DAO

---

## 📎 Посилання на основний WebApp

- 🧭 [AgroProsper WebApp](https://github.com/SerghiiLimborskyi/AgroProsper)  
- 🌐 [Live Site](https://serghiilimborskyi.github.io/AgroProsper/)  
- 🛠️ [Artefact.html – очищення DOM](https://serghiilimborskyi.github.io/AgroProsper/docs/artefact.html)  
- 📥 [Media Kit ZIP](https://github.com/SerghiiLimborskyi/AgroProsper/releases/latest)

---

## 📜 Ліцензія

Цей репозиторій дотримується ліцензії AGRO-LICENSE DAO.  
➡️ [AGRO-LICENSE.uk.txt](https://github.com/SerghiiLimborskyi/AgroProsper/blob/main/AGRO-LICENSE.uk.txt)

---

## 🔍 Для аудиторів та грантодавців

- ✅ Всі компоненти відкриті та перевіряються через GitHub Actions
- 🧪 `SECURITY.md` описує протоколи виявлення вразливостей
- 🔐 Контракти оформлені в `Контракти/AgroChamp.sol`
- 📄 Кожна політика — в `Документи/`, перекладено uk/pl/en
- 📦 ZIP-пакет “AgroProsper Policy Kit” доступний для імпорту
...
- WhatsApp: [`+380XXXXXXXXX`](https://wa.me/380XXXXXXXXX)  
...
# 🆔 NFT Card Generator — AgroProsper DAO

Цей npm-пакет дозволяє генерувати NFT-картки учасника DAO з обраною роллю, бейджем, датою входу і токеном. Картка створюється як SVG або HTML-розмітка — ідеально для інтеграції в WebApp або Telegram-бот.
...
```bash
npm install @serghiilimborskyi/nft-card-generator
